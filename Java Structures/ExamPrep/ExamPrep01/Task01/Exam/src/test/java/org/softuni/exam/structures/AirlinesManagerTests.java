...
public class AirlinesManagerTests {
    ...
    //write test for the completed flights
    @Test
    public void testCompletedFlights_WithData_ShouldReturnCorrectResults() {
        Airline airline = getRandomAirline();
        ...
        assertFalse(set.contains(flight1));
        assertFalse(set.contains(flight2));
        assertFalse(set.contains(flight3));
    }

    @Test
    public void testPerformFlight_WithNoData_ShouldProduceCorrectResults() {
        Airline airline = getRandomAirline();
        Flight flight = getRandomFlight();
        this.airlinesManager.addAirline(getRandomAirline());
        ...
        // Little bit of hacks
        try {
            this.airlinesManager.performFlight(airline, flight);
        } catch (IllegalArgumentException e) {
            assertTrue(true);
            return;
        }
    ...
    @Test
    public void testPerformFlight_WithCorrectData_ShouldProduceCorrectResults() {
        Airline airline = getRandomAirline();
        Flight flight = getRandomFlight();
        this.airlinesManager.addAirline(airline);
        this.airlinesManager.addFlight(airline, flight);

        Flight performedFlight = this.airlinesManager.performFlight(airline, flight);

        assertTrue(performedFlight.isCompleted());
    }
